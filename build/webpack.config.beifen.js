...
let env = require('../cmd/config/release.config');
let version = require('../cmd/config/app.config').version;
let app = require('../cmd/config/app.config').app;
let entry = {};
...
(function walk(dir) {
    ...
    fs.readdirSync(directory).forEach( (file) => {
        ...
        let dir_arr = full_path
            .substring(full_path.indexOf('views') + 6)
            .replace(/\\/g, '/')
            .split('\/');
...
module.exports = {
    entry: entry,
    output: {
        path: path.join(__dirname, `../dist/${app}/${env}/${version}`),
        filename: '[name].js'
    },
    resolve: {
        alias: {
